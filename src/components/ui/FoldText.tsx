...
const HINGE_CONFIG: Record<'top' | 'bottom' | 'left' | 'right', HingeConfig> = {
...
export default function FoldText({
  ...
  useEffect(() => {
    ...
    if (trigger === 'hover') {
      gsap.set(pieces, { opacity: 1, yPercent: 0, rotateX: 0, rotateY: 0, '--fold-crease': 0, transformOrigin: hingeConfig.origin });
  ...
  return (
    <span ref={rootRef} className={`fold-text ${animationType === 'slide' ? 'slide-mode' : ''} ${className}`.trim()} style={rootStyle}>
